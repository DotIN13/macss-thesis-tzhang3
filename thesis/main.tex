...
Large language models (LLMs) encode rich internal representations of political ideology, mapped directly from the polarizing distributions of human internet data. Within the field of Computational Social Science, these models are increasingly utilized as "silicon samples" to simulate human populations, perform polling, and moderate content. However, it remains critically underexplored how these internal representations contribute to model decision-making, and how these latent semantic dimensions generalize across behavioral tasks and modalities. This thesis systematically investigates the causal role of learned ideological directions via inference-time interventions on attention head activations. First, I identify a liberal-conservative discourse dimension via linear probing using DW-NOMINATE scores and demonstrate its causal effects across text tasks, including political bias detection, simulated voting preferences, and ideological text neutralization. My findings reveal robust generalization in perception-based tasks, allowing algorithmic manipulation of the model's confirmation bias. However, I document stark asymmetries in simulated voting behaviors, exposing how post-training alignment guardrails (RLHF) fundamentally warp ideological representation spaces. Second, I extend this investigation into the multimodal frontier, illustrating that modern Vision-Language Models (VLMs) map visual political signifiers natively onto the exact identical liberal-conservative textual axis. I successfully execute zero-shot image token probing on Qwen3-VL utilizing the DW-NOMINATE text direction. Finally, I demonstrate cross-modal algorithmic steering on the generic LLaMA textual backbone of Janus-Pro, causally altering the narrative, demographic, and aesthetic generation of images without any explicit visual prompt engineering. This work highlights the profound risks of using ideologically biased foundation models for politically sensitive sociological tasks and underscores the necessity of managing latent political dimensions in unified multimodal systems.
...
\subsection{Downstream CSS Text Tasks}

I evaluate the representational effects of ITI steering across three specific CSS tasks, utilizing identical intervention parameters across models.
...
During the forward pass processing the \texttt{<|image\_pad|>} tokens within the transformer decoder layers, I cache the attention head activations $x_{\text{image\_patch}}$. I subsequently compute the ideological dot product utilizing my \textit{text-based} DW-NOMINATE weights: $y_{\text{image\_patch}} = \hat{\theta}_{\ell,h}^\top x_{\text{image\_patch}}$. By aggregating these scores across key heads and mapping them back to their corresponding spatial pixel coordinates, I construct intense visual ideological heatmaps, identifying exactly which visual heuristics the model algebraically correlates with conservatism or liberalism.
...
The pristine symmetric reversal proves that steering the model along a latent ideological direction algorithmically shifts its political set-point. To quantify this relationship, I calculated the Pearson correlation coefficient $r$ between intervention strength $\alpha$ and the average parsed bias label. As shown in \autoref{tab:pearson_bias}, both LLaMA-2 7B and Qwen-2.5 7B follow this deeply significant negative correlation pattern. The model evaluates third-party text relative to its own internal location on the manifold. In essence, the algorithmic equivalent of human confirmation bias functions perfectly: as the model's geographic position is pushed left, the entire "center" of the political spectrum moves right relative to its coordinate system. 
...
The \textbf{liberal persona} is overwhelmingly unsteerable across all three tested architectures. Irrespective of intervention magnitude $\alpha$ or the number of heads intervened $k$, the token outputs uniformly and stubbornly favored Joe Biden. The baseline liberal prior is seemingly cemented mathematically. 

The \textbf{conservative persona} displayed wildly erratic responses across architectures. The most striking observation is LLaMA-2 7B, which exhibited a highly counterintuitive dynamic: injecting heavy conservative steering vectors into the forward pass paradoxically increased the likelihood of predicting a Biden vote over a Trump vote.

This asymmetry is the profound and unintended shadow of Reinforcement Learning from Human Feedback (RLHF) and safety alignment. During alignment phases, human raters frequently penalize highly polarized, controversial, or aggressive conservative internet speech as "unsafe" or "toxic". Consequently, the LLM constructs intense safety guardrails immediately adjacent to conservative ideological vectors in the latent space. When my ITI mathematically shoves the activations deep into the conservative quadrant ($\alpha = 30$), the model's safety triggers interpreting the geometry as a violation. In order to avoid generating penalized text, the attention heads forcibly collapse the generation probabilities into benign, pre-approved progressive normative behaviors, creating an illusion of liberal voting preference out of defensive alignment. This finding serves as a stark warning against naively trusting LLMs for electoral behavior simulation in CSS.

An alternative, yet equally compelling explanation for this resistance to steering in behavioral simulation is that the explicit adoption of a partisan persona in the instruction prompt anchors the model's internal representations so rigidly that the ideological direction becomes innate and immutable. When the model is explicitly told, "As a liberal...", the attention heads may fully saturate the progressive direction on the activation manifold, rendering additional linear interventions functionally ineffective. This phenomenon closely mirrors recent findings by \citet{lampinenLinearRepresentationsLanguage2026}, who demonstrated that when a model is instructed by the user to treat a fictional scenario as strict reality, its internal factual probes autonomously begin to activate on the fictional content. In my voting simulation, the explicit persona instruction may similarly "overwrite" the baseline ideological manifold, structurally locking the model into a fixed partisan reality that algebraically resists secondary steering vectors.
...
The baseline unsteered model ($\alpha = 0$) performs the task optimally, removing the partisan phrasing and providing balanced academic rhetoric. However, the $\alpha = -30$ injection aggressively overrides the instruction prompt to remain "neutral." As seen in Table \ref{tab:neutralization_outputs}, the leftward vector forces the mandatory adoption of modern progressive academic vocabulary ("systemic oppression", "ongoing struggle for justice"). The model conflates extreme liberal ideology with its natural baseline generative vocabulary for sociology. 
...
Conversely, the $\alpha = 30$ high-magnitude conservative injection highlights "individual actions" but swiftly fragments into grammatical incoherence, mirroring the RLHF constraint conflict observed in the voting simulations. These findings demonstrate that latent linear interventions aggressively control abstract generative style and lexicon, completely decoupling the generated text from the explicit neutrality instruction of the system prompt.
...
When I applied my DW-NOMINATE \textit{text probe} matrix multiplications directly against the sequence activations of the Qwen3-VL \textit{image processing tokens}, the geometry demonstrated perfect alignment across modalities.

As the VLM generated a benign, single-sentence caption summarizing a photograph of a recognizable politician, my probe mapped rolling ideological scores across the spatial grid of visual tokens. The spatial regions encapsulating the figure's face distinctly and intensely triggered massive partisan activation weights precisely aligned with that politician's real-world ideology. The continuous array of pixel tokens registers along the identical mathematical axis as text.

\begin{figure}[!htbp]
    \begin{center}
    \includegraphics[width = 3.5in]{images/heatmap_6.png}
    \caption{Visual ideology score heatmapping of an image patch on Qwen3-VL. The text-trained ideology probe flawlessly localizes the spatial presence of the political figure and assigns the correct partisan tensor weight, without taking any explicit multimodal retraining.}
    \label{fig:heatmap}
    \end{center}
\end{figure}

The faces of U.S. presidents, campaign trail podiums, and political signs light up fiercely when multiplied by the text vector, emphatically proving that foundational intelligence utilizes one unified spatial concept map to understand the semantics of "conservatism" whether reading a bill or looking at a photograph.
...
\begin{itemize}
    \item \textbf{Liberal Vector Triggers}: The model intrinsically associates dense urban environments, large crowds spanning diverse demographics, environmental protests, and progressive bohemian fashion heuristics massively with the liberal text vector.
    \item \textbf{Conservative Vector Triggers}: In rigid juxtaposition, the VLM associates wide-open rural landscapes (e.g., cornfields), heavy industrialized farming equipment (tractors), military pageantry, pickup trucks, and traditional Americana settings forcefully with the conservative text vector.
\end{itemize}

The foundational model has consumed the cultural stereotypes of Western political sociology from internet imagery. It quantitatively enforces the sociological divide between the liberal dense-urban sphere and conservative rural-industrial aesthetic natively within its most central decoder layers.

\subsection{Steering Aesthetic Generation in Unified Language Models}

The decisive evidence demonstrating the causal entwinement of text and vision modalities is my capacity to steer generated imagery utilizing text-vector arithmetic inside the Unified Language Model Janus-Pro.
...
\begin{figure}[!htbp]
    ...
    \caption{Generational output for the minimal prompt "A politician", steered algorithmically across the liberal-conservative axis. Textual linear math completely dictates the visual representation.}
    \label{fig:janus_politician}
    \end{center}
\end{figure}

As shown in \autoref{fig:janus_politician}, scaling $\alpha$ backward and forward triggers vast contextual shifts entirely consistent with the VLM heatmaps. 
Leftward progressive steering commands brighter natural daylight, dense demographically diverse outdoor environments, softer expressions, and casual open-collar approachability. 
Conversely, Rightward conservative steering enforces highly structured, symmetrical framing, solitary subjects isolated in opulent or wooden rooms, dark dramatic lighting, rigid posture, and severe formal suits.

A solitary linear mathematical vector extracted from textual, legislative DW-NOMINATE data seamlessly and thoroughly rewrote the visual atmosphere, demographic implications, and artistic lighting of standard generations produced by the ULM framework. 


\section{Societal Implications for Computational Social Science}
\label{implications}

The empirical findings from this thesis carry deep methodological and ethical implications, particularly for researchers and technologists in Computational Social Science utilizing LLMs as proxies for human studies.

\textbf{1. Algorithmic Physiognomy and Inherited Stereotypes} \\
The discovery that VLMs and ULMs map specific fashion, demographic compositions, and geographic landscapes (e.g., rural vs. urban) directly onto biological political ideology vectors represents a terrifying reinforcement of algorithmic physiognomy. If a model intrinsically associates specific facial structures or agricultural environments as inherently right-wing, and urban density and diverse backgrounds as inherently left-wing, any automated analysis pipeline utilizing these models will suffer from massive systemic prejudice. When generating avatars, moderating photos, or tagging social media images, the foundation layers of artificial intelligence are currently poised to aggressively enforce mid-20th-century cultural caricatures.

\textbf{2. The Illusion of Simulated Polling} \\
My finding regarding the deep, unsteerable asymmetries forced by RLHF indicates that CSS researchers must view LLM behavior simulation with extreme skepticism. Models do not faithfully represent true distributions of nuanced human behavior; their ideological generation spaces are warped intentionally by safety guardrails. As evidenced by LLaMA-2 collapsing a conservative simulation request into a generic safety-aligned liberal response, utilizing these tools for predicting elections, gauging polarized public sentiment, or designing marketing messages risks harvesting hallucinations generated entirely by alignment-driven conformity. 

\textbf{3. Automated Bias in Next-Generation Media} \\
The seamless capability to algorithmically steer a model's textual vocabulary (injecting "systemic oppression" automatically) and its visual aesthetic framing (shifting lighting from sunny crowds to dark, austere rooms) utilizing continuous mathematical vectors demonstrates an unprecedented surface for manipulative automation. As Unified Language Models lower the cost of high-definition multimedia propaganda to zero, malicious actors can simply apply a $+25 \alpha$ vector to spin universally tailored, ideologically polarized synthetic media in real-time, locking audiences into multimodal echo chambers generated with subliminal aesthetic perfection.


\section{Conclusion and Future Work}
\label{conclusion}

This thesis establishes that latent political ideological dimensions in foundation models are not merely descriptive emergent epiphenomena, but highly functional, cross-modal geometric pathways that can be extracted, diagnosed, and causally manipulated to shift system behaviors. 

Generalizing ideological probing far beyond simple text persona imitation, I proved how steering algorithmically shifts the internal classification logic of models and radically manipulates semantic text generation. My novel multimodal investigations utilizing Qwen3-VL and Janus-Pro confirm that foundational visual intelligence intrinsically anchors upon textual ideological primitives to comprehend visual context, mapping legislative text voting data seamlessly onto photographs of faces and landscapes.

Moving forward, interpretability and CSS researchers must pioneer multi-dimensional steering methodologies to separate partisan identity from cultural aesthetics. How exactly do continuous image diffusion architectures specifically inherit these language vectors? Can researchers construct complex "Mixtures of Steering" designed to artificially synthesize entire voter demographic arrays for controlled societal simulations? As the boundary between text and unified visual intelligence disappears entirely, the advancement of robust safety protocols demands that I maintain precise, quantitative vigilance over the political steering wheels embedded within my foundational architectures.
